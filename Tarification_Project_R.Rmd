...
AIC_data <- data.frame(Modele = c("Modele Poisson", "Modele Binomial Negatif"),
                       AIC = c(AIC(glm_poisson_nb), AIC(glm_bin_neg_nb)))
...
#train <- train[train$ClaimAmount >0,]
#test <- test[test$ClaimAmount >0,]
```
...
glm_gamma_cout <- glm(formula_model,family = Gamma(link="log"), data = train[train$ClaimAmount >0,])
summary(glm_gamma_cout)
```
```{r}
#Modèle après avoir retiré les variables non significatives pour gamma
glm_gamma_cout <- glm(ClaimNb ~ -1 + Gas + Region_1,family = Gamma(link="log"), data = train[train$ClaimAmount >0,])
summary(glm_gamma_cout)
```
...
glm_gaussien_cout <- glm(formula_model,family = gaussian(link="identity"), data = train)
summary(glm_gaussien_cout)
```
on garde l'age du conducteur ainsi que le type d'essence mais vu le peu de significativité des variables, cet modèle ne semble pas intéressant
```{r}
#Modèle en retenant quelques variables significatives
glm_gaussien_cout <- glm(ClaimNb ~ -1 + Gas + DriverAge,family = gaussian(link="identity"), data = train)
summary(glm_gaussien_cout)
```
...
```{r}
# Modèle initial de la Binomiale négative
glm_bin_neg_cout <- glm.nb(formula_model, data = train)
...
glm_bin_neg_cout <- glm.nb(
  ClaimNb ~ -1 + Exposure + Gas +Region_1 + DriverAge +Region_1,data = train)
summary(glm_bin_neg_cout)
```
========== Choix du meilleur modèle selon AIC ==========
...
# Créer un data frame avec les noms de modèle et leur AIC associé
AIC_data <- data.frame(Modele = c("Modele Gamma", "Modele Binomial Negatif"),
                       AIC = c(AIC(glm_gamma_cout), AIC(glm_bin_neg_cout)))
...
y_gaussien_nb = predict(glm_gaussien_cout, newdata = x_test_nb, type = "response")
y_gamma_nb = predict(glm_gamma_cout, newdata = x_test_nb, type = "response")
y_poisson_nb = predict(glm_poisson, newdata = x_test_nb, type = "response")
y_quasipoisson_nb = predict(glm_quasipoisson, newdata = x_test_nb, type = "response")
y_bin_neg_nb = predict(glm_bin_neg, newdata = x_test_nb, type = "response")

# Créer un data frame avec les noms de modèle et leur AIC associé
RMSE_data <- data.frame(Modele = c( "Modele Gamma", "Modele Binomial Negatif"), 
                        RMSE = c(RMSE(y_test_nb, y_gamma_nb),
                                 RMSE(y_test_nb, y_bin_neg_nb)))
...
```{r}
#Montant des sinistres observés : 
mt_sinistres = sum(df$ClaimAmount)

##Modele avec critere AIC
freq_aic<- predict(glm_bin_neg_nb, newdata = base_data_onehot, type = "response")
...
freq_RMSE <- predict(glm_poisson_nb, newdata = base_data_onehot, type = "response")
...
tarif_data <- data.frame(Modele = c( "Modele AIC", "Modele RMSE"), 
                        tarif = c(resultat_aic,resultat_RMSE))

# Afficher le nom du modèle qui minimise le RMSE avec sa valeur
cat("Le tarif choisi par mesure de prudence est le", tarif_data$Modele[which.max(tarif_data$tarif)], "avec tarif de", max(tarif_data$tarif))
