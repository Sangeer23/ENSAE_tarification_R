...
glm_gamma_cout <- glm(ClaimNb ~ -1 + Gas + Region_1,family = Gamma(link="log"), data = train_cout)
summary(glm_gamma_cout)
```
...
# Créer un data frame avec les noms de modèle et leur AIC associé
AIC_data <- data.frame(Modele = c("Modele Gamma","Modele Gaussien", "Modele Binomial Negatif"),
                       AIC = c(AIC(glm_gaussien_cout), AIC(glm_gaussien_cout), AIC(glm_bin_neg_cout)))
...
# Créer un data frame avec les noms de modèle et leur AIC associé
RMSE_data <- data.frame(Modele = c("Modele Gaussien", "Modele Gamma", "Modele Binomial Negatif"), 
                        RMSE = c(RMSE(y_test_nb, y_gaussien_nb),
