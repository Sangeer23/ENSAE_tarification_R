...
train_cout <- base_data_onehot[indexes,]
...
```{r}
x_test_nb = subset(test_freq, select=-c(PolicyID, ClaimNb, ClaimAmount))
y_test_nb = test_freq$ClaimNb
...
train_cout <- train_cout[train_cout$ClaimAmount >0,]
test_cout <- test_cout[test_cout$ClaimAmount >0,]
```
...
glm_gaussien_cout <- glm(formula_model,family = Gamma(link="log"), data = train_cout)
summary(glm_gaussien_cout)
```
```{r}
#Modèle après avoir retiré les variables non significatives pour gamma
glm_gaussien_cout <- glm(ClaimNb ~ -1 + Gas + Region_1,family = Gamma(link="log"), data = train_cout)
summary(glm_gaussien_cout)
```
...
glm_gaussien_cout <- glm(ClaimNb ~ -1 + Gas + DriverAge,family = gaussian(link="identity"), data = train_cout)
summary(glm_gaussien_cout)
```
========== Modele Poisson ========== 
...
```{r}
# Modele initial de la Poisson
glm_poisson_cout <- glm(formula_model,family = poisson(link="log"), data = train_cout)
summary(glm_poisson_cout)
```
A partir de ces informations, on voit qu'on garde toutes les valeurs
...
```{r}
# Modele initial Poisson surdispersée 
glm_quasipoisson_cout <- glm(formula_model,family = quasipoisson(link="log"), data = train_cout)
summary(glm_quasipoisson_cout)
```
Nous retirons les valeurs non significatives et gardons le type d'essence, l'age de la voiture et l'age du conducteur

```{r}
#Modele de poisson surdisperse apres avoir retire les variables non significatives
glm_quasipoisson_cout <- glm(
  ClaimNb ~ -1 + CarAge + Gas + DriverAge, family = quasipoisson(link = "log"),
  data = train_cout)
summary(glm_quasipoisson_cout)
```
========== Modele négatif binomial  ========== 
...
glm_bin_neg_cout <- glm.nb(
  ClaimNb ~ -1 + Exposure + Gas +Region_1 ,data = train_cout)
summary(glm_bin_neg_cout)
```
========== Choix du meilleur modèle selon AIC ==========
...
# Créer un data frame avec les noms de modèle et leur AIC associé
AIC_data <- data.frame(Modele = c("Modele Gamma","Modele Gaussien","Modele Poisson", "Modele Binomial Negatif"),
                       AIC = c(AIC(glm_gaussien_cout), AIC(glm_gaussien_cout), AIC(glm_poisson_cout), AIC(glm_bin_neg_cout)))
